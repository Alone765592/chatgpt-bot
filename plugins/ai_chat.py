...
async def broadcasting_func(client, message):
    ...
    for i , userDoc in enumerate(users_list):
        ...
        if not user_id:
            continue
        try:
            await to_copy_msg.copy(user_id , reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎭 ᴀᴅᴍɪɴ sᴜᴘᴘᴏʀᴛ 🎗️", url='https://t.me/Aesthicunivars')]]))
            completed += 1
